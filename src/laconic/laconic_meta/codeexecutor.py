        ...
        if command.funcproccall() != None:
            ...
            while callBody.VAR(varCounter) != None:
                try:
                    ...
                    pront(
                        "Error: too many arguments in the function call of function "
                        + funcName
                    )
                    raise

                newMapping[
                    functionBeingCalled.args.VAR(varCounter).getText()
            ...
            try:
                ...
                pront(
                    "Error: too few arguments in the function call of function "
                    + funcName
                )
            ...
            while self.evaluateIntexpr(whileLoop.whileexpr().intexpr(), mapping) > 0:
                ...
                if self.executeNonDefProg(
                    whileLoop.whilenondefprog().nondefprog(), mapping
                ):
            ...
            if self.evaluateIntexpr(ifElseState.ifelseexpr().intexpr(), mapping) > 0:
                if self.executeNonDefProg(
                    ifElseState.ifelsenondefprog().nondefprog(), mapping
                ):
                    return True
            else:
                if self.executeNonDefProg(
                    ifElseState.elsenondefprog().nondefprog(), mapping
                ):
                    return True
            return False
            ...
            if command.returnstate().getText() == "halt;":
                try:
                    ...
                    raise Exception(
                        "You can't have a halt statement inside of a function!"
                    )
            ...
            pront(
                mapping[printState.VAR().getText()]
                + ": "
    ...
    def evaluateListexpr(self, ctx, mapping):
        ...
        if ctx.OPERATOR_APPEND() != None:
            ...
            while ctx.intexpr(exprCounter) != None:
                returnList.append(
                    self.evaluateIntexpr(ctx.intexpr(exprCounter), mapping)
                )
                exprCounter += 1
    ...
    def evaluateList2expr(self, ctx, mapping):
        ...
        #        elif ctx.listexpr(0) != None:
            ...
            while ctx.listexpr(exprCounter) != None:
                returnList2.append(
                    self.evaluateListexpr(ctx.listexpr(exprCounter), mapping)
                )
                exprCounter += 1
