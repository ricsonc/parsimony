...
class CodeWriter(LaconicListener):
    ...
    def exitFuncdef(self, ctx):
        ...
        self.funcAuxCountDictionary[self.currentFunc.name] = (
            self.currentFunc.largestFilledReg + 1
        )
        self.currentFunc = self.mainFunc
        self.currentFile = open("main.tmd", "w")
    ...
    def enterFuncproccall(self, ctx):
        ...
        try:
            ...
            pront(
                ...
                + str(list(self.funcProcDictionary.keys()))
            )
            raise
    ...
    def manageRegs(self, ctx, listOfExprs, marker):
        ...
        self.currentFunc.add(
            self.dealWithExpr(
                ctx,
                [ctx.associatedReg] + [expr.associatedReg for expr in listOfExprs],
                marker,
            )
        )
    ...
    def exitWhileexpr(self, ctx):
        ...
        self.currentFunc.add(
            "["
            + stringify(self.currentFunc.filledRegs[0])
            + "] E (WHILE_STATE_"
            + str(ctx.whileCounter)
            + "_FALSE); 1 ()\n"
        )
    ...
    def dealWithExpr(self, ctx, listOfRegs, marker):
        ...
        if marker == "intint":
            ...
            if ctx.INT().getText() == "0":
                ...
                return (
                    "function " + self.BUILTIN_SYMBOL + "clear " + regListInStringForm
                )
            ...
            if ctx.OPERATOR_BOOLEAN() != None:
                ...
                if ctx.OPERATOR_COMPARE().getText() == ">":
                    return self.makeStandardNoteAndLine(
                        "greaterThan", regListInStringForm
                    )
                    ...
                    return (
                        ...
                        + stringify(listOfRegs[0])
                        + " "
                        + stringify(listOfRegs[2])
                        + " "
                        + stringify(listOfRegs[1])
                        + "\n"
                    )

                elif ctx.OPERATOR_COMPARE().getText() == ">=":
                    return self.makeStandardNoteAndLine(
                        "greaterOrEqual", regListInStringForm
                    )
            ...
            return self.makeStandardNoteAndLine(
                "listAssemble" + str(len(listOfRegs) - 1), regListInStringForm
            )

        elif marker == "constlist2":
            return self.makeStandardNoteAndLine(
                "list2Assemble" + str(len(listOfRegs) - 1), regListInStringForm
            )
