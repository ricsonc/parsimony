...
def codeBarf(codeStringInAB):
    listOfStates = [
        State("write_code_symbol_" + str(i), None, alphabetMSToTS())
        for i in range(len(codeStringInAB))
    ]
...
def findAppropriateWordSize(numChars, wordSizeAttempt=2):
    if (
        computeNumCharsTotalFromWordSize(wordSizeAttempt - 1) < numChars
...
def createBlankWord(inState, wordSize, listOfStates):
    ...
    for i in range(wordSize):
        ...
        inStateA, inStateB = createBlankCharacter(
            inStateA, inStateB, "create_blank_char_" + str(i), isFirstChar, listOfStates
        )
...
def convertUnaryToBinary(inState, wordSize, firstDataState, name, listOfStates):
    ...
    doneIncrementing = incrementBinaryCounter(
        increment, wordSize, name + "_incr", listOfStates
    )
...
def introspect(codeStringInAB):
    ...
    inState = organizeDataStates(
        firstDataState, wordSize, numWords, codeStringInAB, listOfStates
    )
    ...
    convertStatesToString(
        listOfStates, open("../../tm/tm2/tm2_files/" + sys.argv[1] + "_prog.tm2", "w")
    )
...
if __name__ == "__main__":
    ...
    codeString = findProgramSkeleton(
        throwawayList,
        throwawayState,
        functions,
        functionVariableDictionary,
