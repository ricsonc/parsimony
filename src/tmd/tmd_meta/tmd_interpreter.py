...
class TuringMachineWithStack:
    def __init__(
        ...
        mainFunctionName = functions[0].strip()

        self.stack = [
            (mainFunctionName, functionVariableDictionary[mainFunctionName], 1)
        ]
        self.lineNumber = 1
        ...
        firstLineOfMainFunction = open(
            path + functions[0].strip() + ".tmd", "r"
        ).readlines()[0]

        for i, varName in enumerate(firstLineOfMainFunction.split()[1:]):
            tape = Tape(varName, "_")
    ...
    def runOneStep(self):
        ...
        if "[" in currentLine:
            ...
            for reaction in reactions:
                ...
                splitReaction = re.split("[(|)|,]", reaction.strip().replace(" ", ""))
                if splitReaction[0] == currentSymbol:
                    ...
                    for command in splitReaction[1:]:
                        if not command in ["1", "E", "_", "L", "R", "-", ""]:
                            try:
                                self.lineNumber = self.functionLabelDictionary[
                                    currentFunction
                                ][command]
                                foundGoto = True
                            except:
                                raise Exception(
                                    "Unrecognized label on line "
                                    + str(self.lineNumberToTextNumber[self.lineNumber])
                                    + " of function "
                                    + currentFunction
                                )
            ...
            if not foundAppropriateReaction:
                raise Exception(
                    "Turing machine threw error on line "
                    + str(self.lineNumberToTextNumber[self.lineNumber])
                    + " of function "
                    + currentFunction
                )
        ...
        elif currentLine.split()[0] == "function":

            oldMappingDict = self.stack[-1][1]

            calledFunction = currentLine.split()[1]
            firstLineOfCalledFunctionSplit = (
                open(self.path + calledFunction + ".tmd", "r").readlines()[0].split()
            )

            argList = currentLine.split()[2:]
            ...
            try:
                ...
                raise Exception(
                    ...
                    + " has "
                    + str(len(argList))
                    + " arguments, but the function being called has "
                    + str(len(firstLineOfCalledFunctionSplit[1:]))
                    + " inputs."
                )
        ...
        elif currentLine.split()[0] == "return":
            ...
            if len(self.stack) == 0:
                pront(
                    "Turing machine halted on line "
                    + str(self.lineNumberToTextNumber[oldLineNumber])
                    + " of function "
                    + currentFunction
                )
                return 1
    ...
    def printAllTapes(self, start, end, output):
        ...
        outString += (
            "Line number: " + str(self.lineNumberToTextNumber[self.lineNumber]) + "\n"
        )
        outString += "\n"

        for tape in list(self.tapeDictionary.values()):
        ...
        for tupIndex in range(len(self.stack) - 1, -1, -1):
            outString += (
                "Function "
                + self.stack[tupIndex][0]
                + " with return address "
                + str(self.lineNumberToTextNumber[self.stack[tupIndex][2]])
                + " and with mapping "
                + str(self.stack[tupIndex][1])
                + "\n"
            )
...
def main():
    ...
    try:
        ...
        raise Exception(
            "Usage: python tmd_interpreter [-q] [-s] [# steps before aborting] [-f] [name of TMD directory]\n \
    ...
    try:
        initVarString = open(path + "initvar", "r").read().strip()
    except:
    ...
    if "-f" in sys.argv:
        ...
        try:
            ...
            raise Exception(
                "You can't include the -f flag without also specifying a maximum step count with the -s flag!"
            )

    else:
        outputFile = None

    TuringMachineWithStack(
        functions,
        path,
        functionLabelDictionary,
        functionLineDictionary,
