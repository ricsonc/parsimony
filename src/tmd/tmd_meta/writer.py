...
def getFunctionLabelDictionary(functions, path):
    ...
    for function in functions:
        ...
        functionLines = open(path + function.strip() + ".tmd", "r")
...
def getFunctionVariableDictionary(functions, path):
    ...
    for function in functions:
        ...
        functionLines = open(path + function.strip() + ".tmd", "r").readlines()

        firstLine = functionLines[0]

        for i, variableName in enumerate(firstLine.split()[1:]):
...
def incrementEachIdentifier(listOfStates, inState):
    ...
    listOfStates.extend(
        [
            ...
            markState,
            findFinState,
            getPastValueLeftState,
            findIdentifierState,
            incrementIdentifierState,
            find_State,
            findValueLeftState,
            findValueRightState,
            getPastValueRightState,
        ]
    )
...
def putHeadsEverywhere(listOfStates, inState):
    ...
    findPattern(
        getToFinState, outState, listOfStates, "write_var_heads", "H_", "R", "R", "H"
    )

    return outState


def writeVariableValuesSkeleton(
    listOfStates, inState, numberOfVariables, initValueString
):
    inState = writeCounter(listOfStates, "write_var", inState, numberOfVariables)
    inState = writeEachVariableValue(
        listOfStates, inState, initValueString, numberOfVariables
    )
...
def writeAuxValues(listOfStates, inState, numberOfVariables, numberOfFunctions):
    inState = writeAuxSkeleton(
        listOfStates, inState, numberOfVariables, numberOfFunctions
    )
...
def writeAuxSkeleton(listOfStates, inState, numberOfVariables, numberOfFunctions):
    ...
    for i in range(int(math.log(numberOfVariables, 2)) + 2):
        listOfLineNumberSpaceStates.append(
            State("write_aux_line_number_space_" + str(i))
        )
    ...
    write_State5 = writeCounter(
        listOfStates, "write_aux", counterInState, numberOfVariables + 1
    )
    listOfStates.append(write_State5)

    listOfFunctionNameSpaceStates = []
    for i in range(int(math.log(numberOfFunctions, 2)) + 2):
        listOfFunctionNameSpaceStates.append(
            State("write_aux_function_name_space_" + str(i))
        )
...
def incrementVariableNames(listOfStates, inState):
    ...
    moveBy(
        goRightTwiceState, name + "_go_right_twice", 2, "R", writeHState1, listOfStates
    )
...
def writeProgramSkeleton(
    ...
    # characteristicString is the binary that will be interpreted.
...
def incrementLineNumberIDs(listOfStates, inState):
    ...
    listOfStates.extend(
        [
            ...
            writeHState,
            findLineNumberState,
            incrementLineNumberState,
            pushEverythingDown_State,
            pushEverythingDown1State,
            pushEverythingDownHState,
            pushEverythingDownEState,
            exitPushState,
            findPushyLineNumberState1,
            findPushyLineNumberState2,
            moveIncrementEnderState1,
            moveIncrementEnderState2,
            checkForFinState,
            getToFinState,
            getToNextFunctionState,
            checkIfLastFunctionState,
        ]
    )
    ...
    findPattern(
        ...
        checkIfLastFunctionState,
        listOfStates,
        "write_code_incr_ln_get_to_next_function",
        "HH",
        "L",
        "L",
        "_",
...
def incrementFunctionIDs(listOfStates, inState):
    ...
    listOfStates.extend(
        [
            ...
            pushEverythingDownEState,
            exitPushState,
            findPushyFunctionNameState1,
            findPushyFunctionNameState2,
            moveIncrementEnderState1,
            moveIncrementEnderState2,
            checkForFinState,
            getToFinState,
...
def write(
    ...
    inState = writeVariableValuesSkeleton(
        listOfStates, inState, numberOfVariables, initValueString
    )
    inState = writeAuxValues(
        listOfStates, inState, numberOfVariables, numberOfFunctions
    )
    inState = writeProgram(
        listOfStates,
        inState,
        functions,
        functionVariableDictionary,
...
def main():
    ...
    try:
        functions = [x.strip() for x in open(path + "functions.tff", "r").readlines()]
    except:
        raise Exception("No functions.tff file found in directory " + path)

    #    print functions.read()

    functionLabelDictionary, functionDictionary = getFunctionLabelDictionary(
        functions, path
    )
