...
class SimpleState:
    ...
    def __hash__(self):
            return hash(self.stateName)
    def __eq__(self, other):
            return self.stateName == other.stateName
...
class FunctionGroup:
    def __init__(
        ...
        listOfBarCodeStates[-1].set3(
            "_", listOfLineGroups[0].inState, "R", functionBarCode[-1]
        )
...
class LineGroup:
    def __init__(
        ...
        if "[" in lineString:
            ...
            listOfBarCodeStates = [
                State(name + "_varname_" + str(i)) for i in range(len(variableBarCode))
            ]
            ...
            listOfBarCodeStates[-1].set3(
                "_", listOfReactionGroups[0].inState, "R", variableBarCode[-1]
            )
            ...
            everythingButLabel = lineString.split(":")[-1]

            splitLine = everythingButLabel.split()

            listOfVarGroups = []

            for variableName in splitLine[2:]:
                listOfVarGroups.append(
                    VarGroup(
                        variableName,
                        functionName,
                        lineNumber,
            ...
            listOfBarCodeStates[-1].set3(
                "_", listOfVarGroups[0].inState, "R", functionBarCode[-1]
            )
...
class ReactionGroup:
    def __init__(
        ...
        if nextLine == None:
            ...
            lineBarCode = convertNumberToBarCode(
                functionLabelDictionary[functionName][nextLine]
            )
            ...
            headMoveState.set3(
                "_", listOfNextLineStates[0], "R", headMoveToSymbol[headMove]
            )
        ...
        listOfStates.extend(
            [self.inState, oneState, readState, writeState, headMoveState]
        )
...
class State(object):
    ...
    def __hash__(self):
        return hash(self.stateName) + hash(tuple(self.alphabet))
    ...
    def __eq__(self, other):
        ...
        for symbol in self.alphabet:
            if (
                self.nextStateDict[symbol].stateName
                != other.nextStateDict[symbol].stateName
            ):
                return False
    ...
    def getNextStateName(self, symbol):
        try:
            ...
            print(
                ("Error: I, state", self.stateName, "don't know about symbol", symbol)
            )
